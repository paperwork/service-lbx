...
onStart() {
    ...
    if [[ ! -f /etc/nginx/nginx.conf ]]; then
        consul-template -consul-addr=localhost:8500 -once -template=/etc/nginx.conf.tmpl:/etc/nginx/nginx.conf
        if [[ $? != 0 ]]; then
            exit 1
        fi
    fi
...
health() {
    ...
    /usr/bin/curl -o /dev/null --fail -s http://127.0.0.1:80/health
    if [[ $? -ne 0 ]]; then
